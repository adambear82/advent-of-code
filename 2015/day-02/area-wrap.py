...
for idx, dims in enumerate(int_dim, start=0): # idx is the index of the current dimension
    ...
    V = d["l"] * d["w"] * d["h"] # V is the volume of the current cuboid
    ...
    dims.remove(max(dims)) # remove the largest dimension
    ribbon = 2 * sum(dims) + V # ribbon is the total ribbon length for the current cuboid + bow
    total_ribbon += ribbon # total_ribbon is the total ribbon length for all cuboids
    print(f"{idx:03}: {dims_display} A:{area:04}, ΣA:{total_area:04}, R:{ribbon:05}, ΣR:{total_ribbon}")
